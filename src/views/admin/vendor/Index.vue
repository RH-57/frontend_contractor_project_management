...
import { useVendors, type Vendor } from '../../../composables/vendor/useVendors';
import { useVendorDelete } from '../../../composables/vendor/useVendorDelete';
import { useToast } from 'vue-toastification';
import { Pencil, Search, Trash2, UserCheck, UserPlus, UserX } from 'lucide-vue-next';
...
const filteredVendors = computed<Vendor[]>(() => {
...
const {mutate} = useVendorDelete()

const handleDelete = (id: number) => {
    if (confirm("Apakah kamu yakin ingin menghapus data vendor ini?")) {
        mutate(id, {
            onSuccess: () => {
                toast.success("Data Vendor berhasil dihapus")
                // Memaksa refetch aktif untuk semua query yang cocok dengan key ['vendors']
...
<template>
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Manajemen Vendor</h1>
                <p class="text-sm text-slate-500 mt-1">Kelola data semua vendor.</p>
            </div>
            <div>
                <router-link
                    to="/vendors/create"
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        ...
                        >
                            ...
                            <td class="py-3.5 px-4">
                                ...
                                >
                                <UserCheck v-if="vendor.is_active" class="w-3.5 h-3.5" />
